...
class LearnViewController: UIViewController, UITableViewDataSource, UITableViewDelegate, AVSpeechSynthesizerDelegate {
    ...
    var speakIndex = 0
    let imgPause = UIImage(named: "speaker_off_ic")
    let imgPlay = UIImage(named: "speaker_ic")
    ...
    override func viewDidLoad() {
        ...
        let nib = UINib(nibName: "LearnTableViewCell", bundle: nil)
    ...
    func tableView(_ tableView: UITableView, cellForRowAt indexPath: IndexPath) -> UITableViewCell {
        let cell = tableView.dequeueReusableCell(withIdentifier: cellId) as! LearnTableViewCell
    ...
    func tableView(_ tableView: UITableView, didSelectRowAt indexPath: IndexPath) {
        if synthesizer.isSpeaking {
            stopSpeaking()
            btnSpeak.setImage(imgPlay, for: .normal)
        }
    ...
    @IBAction func btnSpeak(_ sender: Any) {
        ...
        if !synthesizer.isSpeaking {
            speakAllCell()
            btnSpeak.setImage(imgPause, for: .normal)
        } else {
            stopSpeaking()
            btnSpeak.setImage(imgPlay, for: .normal)
        }
